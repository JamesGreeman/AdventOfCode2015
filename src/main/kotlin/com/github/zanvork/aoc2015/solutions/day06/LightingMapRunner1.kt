...
object LightingMapRunner1 {
    ...
    private fun runInstruction(grid: List<MutableList<Boolean>>, instruction: Instruction) {
        ...
        (start.x .. end.x).forEach { x ->
            (start.y .. end.y).forEach { y ->
                when(instruction.instructionType) {
                    InstructionType.ON -> grid[x][y] = true
                    InstructionType.OFF -> grid[x][y] = false
    ...
    private fun initialiseGrid(): List<MutableList<Boolean>> {
        return List(1000) {
            MutableList(1000) {
                false
            }
